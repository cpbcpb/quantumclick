  ...
  (req, res, next) => {
    Connection.find({ $or: [{ customer: req.user }, { business: req.user }] })
      ...
      .then(listOfConnections => {
        res.render("userViews/myConnections", {
          theUser: req.user,
          listOfConnections
        });
      })
...
router.get("/businesslist", (req, res, next) => {
  User.find()
    .then(listOfBusinesses => {
      Connection.find({
        $and: [{ customer: req.user }, { business: req.body.business }]
      })
        .populate("business")
        .populate("customer")
        .then(alreadyExists => {
        ...
        .catch(err => {
          console.log("error from comparision"), next(err);
        });
    })
    .catch(err => {
      next(err);
...
router.post("/business/createconnect", (req, res, next) => {
  Connection.findOne({
    $and: [{ business: req.body.business }, { customer: req.user._id }]
  }).then(thisthing => {
    ...
    if (thisthing !== null) {
      Connection.find({ $or: [{ customer: req.user }, { business: req.user }] })
        ...
        .then(listOfConnections => {
          res.render("userViews/myConnections", {
            theUser: req.user,
            Message: "You are connected to this business"
          });
        })
        .catch(err => {
          next(err);
...
router.post("/business/approve", (req, res, next) => {
  User.update(
    { _id: req.user._id },
    { $push: { connections: req.body.connectionId } }
  );
